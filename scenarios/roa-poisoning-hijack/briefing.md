...
This is a two-handed move. The trust half pulls the ROA. The routing half (you, on
the foothold) makes the announcement once the cover is gone. The point is that the
attack on the routing system can begin in the trust system, hours or days earlier,
and the two only make sense read together.

## The shape

1. ROV is on. The /25 hijack is RPKI-invalid and dropped. Nothing works yet.
2. The ROA is withdrawn (the arming). 203.0.113.0/24 becomes not-found.
3. You announce 203.0.113.0/25. It is now not-found, ROV permits it, and it wins
   by longest-prefix. The hijack you could not land before lands now.
